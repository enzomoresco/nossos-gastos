...
function render(){
  ...
  for(const x of DATA){
    const v = Number(x.valor)||0;
    total += v;
    if(toISO(x.data).slice(0,7) === ym) totalMonth += v;
...
function renderEntries(){
  ...
  const sorted = [...DATA].sort((a,b)=> toISO(b.data).localeCompare(toISO(a.data)));
...
// Normaliza QUALQUER data ("2026-01-05" ou "Mon Jan 05 2026 ...") para "2026-01-05"
function toISO(v){
  if(!v) return "";
  const s = String(v);
  // já está no formato certo?
  if(/^\d{4}-\d{2}-\d{2}/.test(s)) return s.slice(0,10);
  // tenta interpretar como data (ex: "Mon Jan 05 2026 00:00:00 GMT-0300")
  const d = new Date(s);
  if(!isNaN(d)){
...
function formatDay(v){
  ...
  try{
    ...
    if(isNaN(d)) return iso || "Sem data";
